...
public class GcmTaskTagExecutorTest {
    ...
    @Test public void executeMatchesRefresh() throws Exception {
        ...
        assertEquals(GcmNetworkManager.RESULT_SUCCESS, executor.execute("com.globo.sync.tag.SYNC_MATCHES", null));
        when(matchRepositoryMock.refreshMatches()).thenReturn(Observable.<Void>error(new IOException()));
        assertEquals(GcmNetworkManager.RESULT_FAILURE, executor.execute("com.globo.sync.tag.SYNC_MATCHES", null));
    }

    @Test public void executeMatchLiveTickerRefresh() throws Exception {
        final Bundle bundle = new Bundle();
        bundle.putInt("com.globo.sync.extra.EXTRA_MATCH_ID", 1);
        when(matchLiveTickerRepositoryMock.refreshMatchLiveTicker(1)).thenReturn(Observable.<Void>empty());
        assertEquals(GcmNetworkManager.RESULT_SUCCESS, executor.execute("com.globo.sync.tag.SYNC_MATCH_LIVE_TICKER", bundle));
        when(matchLiveTickerRepositoryMock.refreshMatchLiveTicker(1)).thenReturn(Observable.<Void>error(new IOException()));
        assertEquals(GcmNetworkManager.RESULT_FAILURE, executor.execute("com.globo.sync.tag.SYNC_MATCH_LIVE_TICKER", bundle));
    }
}