...
public class SyncSchedulerManagerTest {
    ...
    @Test public void syncMatches() throws Exception {
        ...
        when(gcmTaskFactoryMock.createPeriodicTask(tag, 49, serviceClass, null)).thenReturn(periodicTaskMock);
    ...
    @Test public void syncMatchLiveTicker() throws Exception {
        ...
        PeriodicTask periodicTaskMock = mock(PeriodicTask.class);
        when(gcmTaskFactoryMock.createPeriodicTask(eq(tag), eq(80L), eq(serviceClass), any(Bundle.class))).thenReturn(periodicTaskMock);
        scheduler.syncMatchLiveTicker(1, 80);

        verify(gcmTaskFactoryMock).createPeriodicTask(eq(tag), eq(80L), eq(serviceClass), any(Bundle.class));
        verify(gcmNetworkManagerMock).schedule(periodicTaskMock);
    }

    @Test public void cancelMatchesLiveTickerSync() throws Exception {
        scheduler.cancelMatchesLiveTickerSync();
        verify(gcmNetworkManagerMock).cancelTask("com.globo.sync.tag.SYNC_MATCH_LIVE_TICKER", serviceClass);
    }
}