...
@RunWith(AndroidJUnit4.class) public class MatchSQLiteRepositoryIntegrationTest {

  private MatchSQLiteRepository repository;

  @Before public void setUp() throws Exception {
    repository = new MatchSQLiteRepository(SqlBrite.create()
        .wrapDatabaseHelper(new BrasileiraoDatabaseHelper(
  ...
  @Test public void saveMatchAndQuery() throws Exception {
    ...
    final Match match = new Match(1, new Team("Grêmio", "www.host.com/gremioIcon"),
        new Team("Internacional", "www.host.com/interIcon"), 2, 0, calendar.getTime(),
    ...
    assertEquals(match, testSubscriber.getOnNextEvents()
        .get(0));
    testSubscriber.assertNotCompleted();

    final Match modifiedMatch = new Match(1, new Team("Grêmio", "www.host.com/gremioIcon"),
        new Team("Flamengo", "www.host.com/flamengoIcon"), 2, 0, calendar.getTime(),
        "Sociedade Leopoldina Porto Alegrense");
  ...
  @Test public void saveMatchesAndQuery() throws Exception {
    ...
    final List<Match> matches = Arrays.asList(
        ...
        new Match(2, new Team("Flamengo", "www.host.com/flamengoIcon"),
            new Team("Vasco", "www.host.com/vascoIcon"), 5, 3, calendar.getTime(), "Maracanã"));
    ...
    testSubscriber.assertNoErrors();
    testSubscriber.assertValueCount(1);
    assertEquals(matches, testSubscriber.getOnNextEvents()
        .get(0));
    testSubscriber.assertNotCompleted();
    testSubscriber.unsubscribe();
    testSubscriber.assertUnsubscribed();
    ...
    assertTrue(testSubscriber4.getOnNextEvents()
        .get(0)
        .isEmpty());
  ...
  @Test public void saveLiveTickerEntriesAndQuery() throws Exception {
    final List<Match> matches = Collections.singletonList(
        new Match(1, new Team("Grêmio", "www.host.com/gremioIcon"),
            new Team("Internacional", "www.host.com/interIcon"), 2, 0, new Date(),
            "Sociedade Leopoldina Porto Alegrense"));
    repository.saveOrOverwriteMatches(matches);

    final List<LiveTickerEntry> liveTickerEntries =
        Arrays.asList(new LiveTickerEntry(1, 23, "Test 1"), new LiveTickerEntry(1, 45, "Test 2"));
    ...
    repository.getLiveTickerEntries(1)
        .subscribe(testSubscriber);
    testSubscriber.assertNoErrors();
    testSubscriber.assertValueCount(1);
    assertEquals(liveTickerEntries.get(1), testSubscriber.getOnNextEvents()
        .get(0)
        .get(0));
    testSubscriber.assertNotCompleted();

    final List<LiveTickerEntry> newLiveTickerEntries =
        Arrays.asList(new LiveTickerEntry(1, 60, "Test 3"), new LiveTickerEntry(1, 61, "Test 4"));
    ...
    assertTrue(testSubscriber2.getOnNextEvents()
        .get(0)
        .isEmpty());
