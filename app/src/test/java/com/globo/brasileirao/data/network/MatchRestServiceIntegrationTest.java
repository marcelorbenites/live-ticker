...
public class MatchRestServiceIntegrationTest {
    ...
    @Test public void getLiveTickerEntriesSuccess() throws Exception {
        ...
        retrofit.create(MatchRestService.class).getLiveTickerEntries("{\"matchId\":1}", "MyApiKey").subscribe(testSubscriber);
        testSubscriber.assertNoErrors();
        testSubscriber.assertValueCount(1);
        ...
        RecordedRequest request = server.takeRequest();
        assertEquals("/api/1/databases/heroku_wm3w0h9v/collections/liveTickerEntries?q={%22matchId%22:1}&apiKey=MyApiKey", request.getPath());
        assertEquals("GET", request.getMethod());
