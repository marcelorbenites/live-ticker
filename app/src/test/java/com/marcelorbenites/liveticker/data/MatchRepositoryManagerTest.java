...
@RunWith(MockitoJUnitRunner.class) public class MatchRepositoryManagerTest {
  ...
  @Test public void refreshMatchesNetworkError() throws Exception {
    ...
    when(diskRepositoryMock.getMatches()).thenReturn(
        Observable.just(Collections.<Match>emptyList()));
  ...
  @Test public void getMatchesNetworkSuccess() throws Exception {
    ...
    when(networkRepositoryMock.getMatches()).thenReturn(Observable.just(resultList));
    when(diskRepositoryMock.getMatches()).thenReturn(
        Observable.just(Collections.<Match>emptyList()));
  ...
  @Test public void refreshLiveTickerNetworkError() throws Exception {
    ...
    when(diskRepositoryMock.getLiveTickerEntries(1)).thenReturn(
        Observable.just(Collections.<LiveTickerEntry>emptyList()));

    TestSubscriber<Void> testSubscriber = new TestSubscriber<>();
    repository.refreshLiveTickerEntries(1)
        .subscribe(testSubscriber);
    testSubscriber.assertError(IOException.class);
    testSubscriber.assertNoValues();
    testSubscriber.assertUnsubscribed();
  ...
  @Test public void refreshLiveTickerSkip0() throws Exception {
    List<LiveTickerEntry> resultList =
        Arrays.asList(new LiveTickerEntry(1, 23, "test 1"), new LiveTickerEntry(1, 45, "test 2"));

    when(networkRepositoryMock.getLiveTickerEntries(1)).thenReturn(Observable.just(resultList));
    when(diskRepositoryMock.getLiveTickerEntries(1)).thenReturn(
        Observable.just(Collections.<LiveTickerEntry>emptyList()));
  ...
  @Test public void refreshLiveTickerSkip1() throws Exception {
    ...
    when(networkRepositoryMock.getLiveTickerEntries(1)).thenReturn(Observable.just(resultList));
    when(diskRepositoryMock.getLiveTickerEntries(1)).thenReturn(
        Observable.just(Collections.singletonList(new LiveTickerEntry(1, 23, "test 0"))));
