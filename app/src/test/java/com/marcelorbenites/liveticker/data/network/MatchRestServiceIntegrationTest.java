...
public class MatchRestServiceIntegrationTest {
  ...
  @Test public void getMatchesSuccess() throws Exception {
    ...
    final Retrofit retrofit = new Retrofit.Builder().baseUrl(server.url("/"))
    ...
    retrofit.create(MatchRestService.class)
        .getMatches("MyApiKey")
        .subscribe(testSubscriber);
    testSubscriber.assertNoErrors();
    testSubscriber.assertValueCount(1);
    assertEquals(10, testSubscriber.getOnNextEvents()
        .get(0)
        .size());
    testSubscriber.assertCompleted();

    RecordedRequest request = server.takeRequest();
    assertEquals("/api/1/databases/heroku_wm3w0h9v/collections/matches?apiKey=MyApiKey",
        request.getPath());
  ...
  @Test public void getLiveTickerEntriesSuccess() throws Exception {
    ...
    retrofit.create(MatchRestService.class)
        .getLiveTickerEntries("{\"matchId\":1}", "MyApiKey")
        .subscribe(testSubscriber);
    testSubscriber.assertNoErrors();
    testSubscriber.assertValueCount(1);
    assertEquals(3, testSubscriber.getOnNextEvents()
        .get(0)
        .size());
    testSubscriber.assertCompleted();
    ...
    RecordedRequest request = server.takeRequest();
    assertEquals(
        "/api/1/databases/heroku_wm3w0h9v/collections/liveTickerEntries?q={%22matchId%22:1}&apiKey=MyApiKey",
        request.getPath());
    assertEquals("GET", request.getMethod());
  ...
  @Test public void getMatch() throws Exception {
    ...
    retrofit.create(MatchRestService.class)
        .getMatch("{\"matchId\":1}", "MyApiKey")
        .subscribe(testSubscriber);
    testSubscriber.assertNoErrors();
    testSubscriber.assertValueCount(1);
    assertEquals(1, testSubscriber.getOnNextEvents()
        .get(0)
        .getMatchId());
    assertEquals(5, testSubscriber.getOnNextEvents()
        .get(0)
        .getHomeScore());
    assertEquals(1, testSubscriber.getOnNextEvents()
        .get(0)
        .getAwayScore());
    assertEquals("Maracanã", testSubscriber.getOnNextEvents()
        .get(0)
        .getLocation());
    testSubscriber.assertCompleted();
    ...
    RecordedRequest request = server.takeRequest();
    assertEquals(
        "/api/1/databases/heroku_wm3w0h9v/collections/matches?fo=true&q={%22matchId%22:1}&apiKey=MyApiKey",
        request.getPath());
    assertEquals("GET", request.getMethod());
