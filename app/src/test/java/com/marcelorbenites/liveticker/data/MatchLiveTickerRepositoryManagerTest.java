...
public class MatchLiveTickerRepositoryManagerTest {
  ...
  @Test public void refreshLiveTicker() throws Exception {
    ...
    repository.refreshMatchLiveTicker(34)
  ...
  @Test public void getMatchLiveTicker() throws Exception {
    ...
    List<LiveTickerEntry> liveTickerEntries =
        Arrays.asList(new LiveTickerEntry(5, 5, ""), new LiveTickerEntry(5, 15, ""),
            new LiveTickerEntry(5, 30, ""));
    when(matchRepositoryMock.getLiveTickerEntries(5)).thenReturn(
        Observable.just(liveTickerEntries));

    TestSubscriber<MatchLiveTicker> testSubscriber = new TestSubscriber<>();
    repository.getMatchLiveTicker(5)
        .subscribe(testSubscriber);
    testSubscriber.assertNoErrors();
