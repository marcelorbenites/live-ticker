...
/**
 * Class that creates and handles updates of Aurea database later being accessed by
 * AureaContentProvider.
 */
public class BrasileiraoDatabaseHelper extends SQLiteOpenHelper {
  ...
  private static final String CREATE_TABLE_MATCHES = String.format("create table %s ("
          ...
          + "%s int not null,"
          + "%s int not null,"
          + "%s text not null);", "matches", "matchId", "homeTeamName", "homeTeamIconUrl",
      "awayTeamName", "awayTeamIconUrl", "homeScore", "awayScore", "date", "location");
  ...
  private static final String CREATE_TABLE_LIVE_TICKER_ENTRIES = String.format("create table %s ("
          ...
          + "%s text not null, "
          + "primary key (%s, %s),"
          + "foreign key (%s) references %s(%s));", "live_ticker_entries", "time", "matchId",
      "description", "time", "matchId", "matchId", "matches", "matchId");
