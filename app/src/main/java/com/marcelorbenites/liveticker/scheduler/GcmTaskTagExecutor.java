...
public class GcmTaskTagExecutor {
  ...
  @Inject public GcmTaskTagExecutor(MatchRepository matchRepository,
      MatchLiveTickerRepository matchLiveTickerRepository) {
  ...
  public int execute(String tag, Bundle bundle) {
    switch (tag) {
      ...
      case SyncSchedulerManager.TAG_SYNC_MATCH_LIVE_TICKER:
        return matchLiveTickerRepository.refreshMatchLiveTicker(
            bundle.getInt(SyncSchedulerManager.EXTRA_MATCH_ID))
            .cast(Integer.class)
            .onErrorResumeNext(Observable.just(GcmNetworkManager.RESULT_FAILURE))
            .toBlocking()
            .firstOrDefault(GcmNetworkManager.RESULT_SUCCESS);
